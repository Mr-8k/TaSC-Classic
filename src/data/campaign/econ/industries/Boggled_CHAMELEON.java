...
public class Boggled_CHAMELEON extends BaseIndustry
{
    ...
    private int requiredDaysToRemoveDeciv = 200;
    ...
    public void advance(float amount)
    {
        ...
        if(this.market.hasCondition("decivilized_subpop") && this.isFunctional())
        {
            ...
            if(clock.getDay() != lastDayChecked && !shortage)
            {
                daysWithoutShortage++;
                lastDayChecked = clock.getDay();

                if(daysWithoutShortage >= requiredDaysToRemoveDeciv)
                {
    ...
    protected void addRightAfterDescriptionSection(TooltipMakerAPI tooltip, IndustryTooltipMode mode)
    {
        ...
        if(this.isDisrupted() && this.market.hasCondition("decivilized_subpop") && mode != IndustryTooltipMode.ADD_INDUSTRY && mode != IndustryTooltipMode.QUEUED && !isBuilding())
        {
